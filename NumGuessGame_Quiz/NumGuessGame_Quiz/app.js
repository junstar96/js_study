...
function displayGuesses(guess) {
  ...
  $remainingCount.innerHTML = $remainingCount.innerHTML - 1;
}

// 유저에게 띄울 메세지를 입력합니다.
function displayMessage(message) {
  // 구현 2. 유저에게 메세지를 보여줄 수 있도록 아래의 영역을 구현해주세요
  let show_text = `<h1>${message}</h1>`
  $guessingResult.innerHTML = show_text;
}
...
async function makeAnswerCircle(guess) {
  ...
  let check = await showCircle(guess, CIRCLE_NAME, $answerCircleArea);

  check.id = "answerCircle";
  check.append(CIRCLE_NAME);
...
async function makeGuessCircle(guess) {
  ...
  let check = await showCircle(guess, CIRCLE_NAME, $guessCircleArea);

  check.id = "guessCircle";
  check.append(CIRCLE_NAME);
  // 구현3. 유저가 원의 크기로 정답을 유추하기 쉽도록 showCircle 함수를 이용해서 해당 부분을 구현해주세요,
  // showCircle 함수의 "작업"이 끝나면, 해당 div에 원의 이름을 입력해주세요
}
